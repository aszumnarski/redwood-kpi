package com.redwood.scheduler.custom.kpi.kpi3.model;

import com.redwood.scheduler.api.model.Job;
import com.redwood.scheduler.api.model.SchedulerSession;
import com.redwood.scheduler.api.date.DateTimeZone;

import java.io.PrintWriter;

class AccountItemAuto
{
    private final LeafCollector collector;

    public AccountItemAuto(Job job,DateTimeZone parentDate)
            throws Exception
    {
        collector = new LeafCollector(job, parentDate, CollectorConfigs.AUTO);
    }

    public AccountItemAuto(AccountItemSource source) {
        collector = new LeafCollector(source.getContext(), source.getStats());
    }

    public void collectChildren(SchedulerSession session, PrintWriter p,Job job)
            throws Exception
    {
        collector.collectChildren(session, p, job);
    }

    public LeafCollector getCollector(){
        return collector;
    }

}